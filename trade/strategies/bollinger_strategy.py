    ...
    def next2(self):
        ...
        if not self.position:
            if self.data.close > self.boll.lines.top:
                self.order = self.sell(exectype=bt.Order.Stop, price=self.boll.lines.top[0])  # , size=self.p.size)
            ...
            if self.position.size > 0:
                ...
                self.order = self.buy(exectype=bt.Order.Limit, price=self.boll.lines.mid[0])
