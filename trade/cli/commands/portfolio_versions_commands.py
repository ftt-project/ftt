...
class PortfolioVersionsCommands:
    ...
    @command
    @argument("portfolio_version_id", description="Portfolio Version ID", positional=True)
    ...
    @argument("interval", description="Trading interval", choices=["1m", "5m", "15m", "1d", "1wk", "1mo"])
    def balance(self, portfolio_version_id: int, period_start: str, period_end: str, interval: str) -> None:
        """
        ...
        if self.portfolio_in_use is None:
            ctx.console.print("[yellow]Select portfolio using `portfolio use ID` command")
            return

        portfolio_result = PortfolioLoadHandler().handle(portfolio_id=self.portfolio_in_use)
        ...
        if weights_result.is_err():
            ctx.console.print(f"[red]:disappointed: Failed to calculate weights for this portfolio:")
            ctx.console.print(f"    [red]:right_arrow: {weights_result.err().value}")
            return

        result = WeightsListHandler().handle(portfolio_version=portfolio_version_result.value)
