...
class SecuritiesAssociateWithPortfolioStep(AbstractStep):
    ...
    """
    @classmethod
    def process(cls, portfolio: schemas.Portfolio, securities: list[schemas.Security]) -> Result[list[schemas.Security], str]:
        associated_securities: list[schemas.Security] = []
        for security in securities:
            security: schemas.Security = SecuritiesRepository.get_by_name(security.symbol)
            result: schemas.PortfolioSecurity = PortfolioSecurityRepository.associate(portfolio, security)
